...
public class MainActivity extends Activity implements SurfaceHolder.Callback {
                ...
                if (cfg || info.size <= 0) {
                    if (cfg && mRecording && mPreviewDecoder != null) {
                        ...
                        if (data != null) {
                            ...
                            sendToPreviewDecoder(ByteBuffer.wrap(bytes), info);
                        }
                    }
                    continue;
                }
    ...
    private void drainPreviewDecoder(MediaCodec dec) {
        MediaCodec.BufferInfo info = new MediaCodec.BufferInfo();
        while (true) {
            int out = dec.dequeueOutputBuffer(info, 0);
            if (out < 0) break;
    ...
    private void doStart() {
        try {
            ...
            try {
                ...
                mPreviewDecoder.configure(decoderFormat, mMonitorView.getHolder().getSurface(), null, 0);
                mPreviewDecoder.start();
            } catch (Exception e) {
                status("Preview decoder err: " + e.getMessage());
    ...
    private void finalizeMuxer() {
        ...
        if (mPreviewDecoder != null) {
            try { mPreviewDecoder.stop(); mPreviewDecoder.release(); } catch (Exception ignored) {}
            mPreviewDecoder = null;
        }
        runOnUiThread(() -> {
            mBtn.setText("⏺ REC"); mBtn.setBackground(mBtnBgIdle); mBtn.setEnabled(true);
