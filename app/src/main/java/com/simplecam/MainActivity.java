...
public class MainActivity extends Activity implements SurfaceHolder.Callback {
	...
	protected void onDestroy() {
		super.onDestroy();
		// Снимаем ВСЕ pending задачи camera-потока, включая startPreview
		// запостенный stopEisOverlay() в onPause(). Иначе quitSafely()
		// дренирует очередь и startPreview выполнится на закрытой камере → crash.
		if (mCamHandler != null) mCamHandler.removeCallbacksAndMessages(null);
		mVidLoopRunning = false;
		stopAudio();
		finalizeMuxer();
		// Порядок важен: сначала закрываем сессию/камеру, потом encoder-поверхность.
		// Иначе сессия может писать во уже освобождённую поверхность → native crash.
	...
	public void surfaceCreated(SurfaceHolder h) {
		// Фиксируем буфер превью = размеру encoder-поверхности.
		// Разные размеры двух PRIV-стримов заставляют некоторые HAL
		// применять EIS в разных масштабных пространствах — дополнительный
		// источник смещения. С одинаковыми размерами HAL использует единый
		// EIS-пайплайн для обоих стримов.
	...
	private void buildAndSendRequest() {
		...
		try {
			...
			// ── Зум: SCALER_CROP_REGION нельзя использовать вместе с EIS ──────────
			// Camera2 spec: "Applications should not set SCALER_CROP_REGION while
			// VIDEO_STABILIZATION_MODE_ON is set." Большинство Qualcomm/MediaTek HAL
			// применяют EIS-трансформ относительно угла (0,0) запрошенного
			// crop-региона вместо его центра → несимметричная обрезка в файле.
			// При зуме хуже: crop-регион меньше, и тот же pixel-offset — бо́льшая
			// доля кадра.
			//
			// Решение:
			//  • EIS ON  + API 30+: CONTROL_ZOOM_RATIO — EIS-совместим по spec.
			//  • EIS ON  + API <30: зум недоступен (не трогаем ни один параметр).
			//  • EIS OFF           : SCALER_CROP_REGION как прежде.
			boolean eisActive = mEisEnabled && mEisSupported;
			if (eisActive) {
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
					// CONTROL_ZOOM_RATIO (API 30+) обрабатывается HAL до EIS-пайплайна,
					// поэтому стабилизатор всегда работает в правильном координатном
					// пространстве независимо от уровня зума.
					rb.set(CaptureRequest.CONTROL_ZOOM_RATIO, mZoomLevel);
					// SCALER_CROP_REGION НЕ устанавливаем
				}
				// API <30: оба параметра не трогаем — HAL сам выбирает EIS-crop.
			} else if (mSensorRect != null) {
				// EIS выключен: классический центрированный crop для зума.
